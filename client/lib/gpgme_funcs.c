...
int
init_gpgme(ztn_ctx_t ztn_ctx)
{
    gpgme_error_t       err;

    /* 如果我们已经有了背景，我们就完了。 */
    if(ztn_ctx->have_gpgme_context)
        return(ZTN_SUCCESS);
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        /* GPG发动机不可用。 */
        ztn_ctx->gpg_err = err;
        return(ZTN_ERROR_GPGME_NO_OPENPGP);
    }

    /* 提取当前gpgme引擎信息。 */
    gpgme_set_engine_info(
            GPGME_PROTOCOL_OpenPGP,
            (ztn_ctx->gpg_exe != NULL) ? ztn_ctx->gpg_exe : GPG_EXE,
            ztn_ctx->gpg_home_dir   /* 如果为NULL，则使用默认值 */
    );

    /* 创建我们的gpgme上下文 */
    err = gpgme_new(&(ztn_ctx->gpg_ctx));
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ztn_ctx->gpg_err = err;
        return(ZTN_ERROR_GPGME_CONTEXT);
    }

    ztn_ctx->have_gpgme_context = 1;
...
static int
process_sigs(ztn_ctx_t ztn_ctx, gpgme_verify_result_t vres)
{
    ...
    while(sig != NULL)
    {
        fgs = calloc(1, sizeof(struct ztn_gpg_sig));
        if(fgs == NULL)
        ...
        if(sig_cnt == 0)
            ztn_ctx->gpg_sigs = fgs;
        else
            ztn_ctx->gpg_sigs->next = fgs;
    ...
    if(ztn_ctx->ignore_gpg_sig_error != 0)
        return(ZTN_SUCCESS);

    /* 否则，我们将在此处进行检查并做出相应的回应。 */
    fgs = ztn_ctx->gpg_sigs;

    if(fgs->status != GPG_ERR_NO_ERROR || fgs->validity < 3) {
        ztn_ctx->gpg_err = fgs->status;

        return(ZTN_ERROR_GPGME_BAD_SIGNATURE);
    }
...
int
get_gpg_key(ztn_ctx_t ztn_ctx, gpgme_key_t *mykey, const int signer)
{
    int             res;
    const char     *name;
    ...
    /* 为列表创建gpgme上下文 */
    /* 初始化gpgme */
    res = init_gpgme(ztn_ctx);
    if(res != ZTN_SUCCESS)
    {
        if(signer)
            return(ZTN_ERROR_GPGME_CONTEXT_SIGNER_KEY);
        else
            return(ZTN_ERROR_GPGME_CONTEXT_RECIPIENT_KEY);
    }

    list_ctx = ztn_ctx->gpg_ctx;

    if(signer)
        name = ztn_ctx->gpg_signer;
    else
        name = ztn_ctx->gpg_recipient;
    ...
    if (err)
    {
        ...
        if(signer)
            return(ZTN_ERROR_GPGME_SIGNER_KEYLIST_START);
        else
            return(ZTN_ERROR_GPGME_RECIPIENT_KEYLIST_START);
    }
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ...
        if(signer)
            return(ZTN_ERROR_GPGME_SIGNER_KEY_NOT_FOUND);
        else
            return(ZTN_ERROR_GPGME_RECIPIENT_KEY_NOT_FOUND);
    }
    ...
    if(gpg_err_code(err) == GPG_ERR_NO_ERROR) /* 注意：查找无错误 */
    {
        ...
        if(signer)
            return(ZTN_ERROR_GPGME_SIGNER_KEY_AMBIGUOUS);
        else
            return(ZTN_ERROR_GPGME_RECIPIENT_KEY_AMBIGUOUS);
    }
...
/* libztn的主要GPG加密例程。 */
int
gpgme_encrypt(ztn_ctx_t ztn_ctx, unsigned char *indata, size_t in_len,
        const char *pw, unsigned char **out, size_t *out_len)
{
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ...
        ztn_ctx->gpg_ctx = NULL;
        ztn_ctx->gpg_err = err;

        return(ZTN_ERROR_GPGME_PLAINTEXT_DATA_OBJ);
    }
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ...
        return(ZTN_ERROR_GPGME_SET_PROTOCOL);
    }
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ...
        return(ZTN_ERROR_GPGME_CIPHER_DATA_OBJ);
    }

    /* 在这里，我们将签名者添加到gpgme上下文中（如果有的话）。 */
    if(ztn_ctx->gpg_signer != NULL) {
        gpgme_signers_clear(gpg_ctx);
        err = gpgme_signers_add(gpg_ctx, ztn_ctx->signer_key);
        if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
        {
            gpgme_data_release(plaintext);
            ...
            return(ZTN_ERROR_GPGME_ADD_SIGNER);
        }
    }
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ...
        if(gpgme_err_code(err) == GPG_ERR_CANCELED)
            return(ZTN_ERROR_GPGME_BAD_PASSPHRASE);

        return(ZTN_ERROR_GPGME_ENCRYPT_SIGN);
    }
    ...
    *out = calloc(1, *out_len); /* 这是在ztn_ctx销毁后释放的。 */
    if(*out == NULL)
        res = ZTN_ERROR_MEMORY_ALLOCATION;
    else
    {
...
/* libztn的主要GPG解密例程。 */
int
gpgme_decrypt(ztn_ctx_t ztn_ctx, unsigned char *indata,
        size_t in_len, const char *pw, unsigned char **out, size_t *out_len)
{
    ...
    if(gpg_err_code(err) != GPG_ERR_NO_ERROR)
    {
        ...
        return(ZTN_ERROR_GPGME_DECRYPT_FAILED);
    }
    ...
    if(decrypt_res->unsupported_algorithm)
    {
        ...
        return(ZTN_ERROR_GPGME_DECRYPT_UNSUPPORTED_ALGORITHM);
    }

    /* 现在验证签名（如果已配置）。 */
    if(ztn_ctx->verify_gpg_sigs)
    {
        verify_res  = gpgme_op_verify_result(gpg_ctx);

        res = process_sigs(ztn_ctx, verify_res);
    ...
    *out = calloc(1, *out_len+1); /* 这是在ztn_ctx销毁后释放的。 */
