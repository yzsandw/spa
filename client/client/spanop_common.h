/**
 * \file client/spa_common.h
 *
 * \brief Header file for spa config_init.
 */
...
//spa客户端配置参数和值
typedef struct ztn_cli_options
{
    char config_file[MAX_PATH_LEN]; //配置文件的路径
    char access_str[MAX_PATH_LEN]; //访问字符串
    char rc_file[MAX_PATH_LEN]; //指定 spa rc 文件的路径
    ...
    char save_packet_file[MAX_PATH_LEN]; //指示 spa 客户端将新创建的 SPA 数据包写出到指定的文件的路径
    ...
    char spoof_ip_src_str[MAX_IPV4_STR_LEN]; //欺骗 spa 客户端发送 SPA 数据包的源地址
    char spoof_user[MAX_USERNAME_LEN]; //欺骗 spa 客户端发送 SPA 数据包的用户名
...
void free_configs(ztn_cli_options_t *opts);
