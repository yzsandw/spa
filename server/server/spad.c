...
static void daemonize_process(ztn_srv_options_t * const opts);

//2023.7.19 zxj 停止运行的spad进程
static int stop_spad(ztn_srv_options_t * const opts);
//2023.7.19 zxj 获取spad进程的状态
static int status_spad(ztn_srv_options_t * const opts);

//2023.7.19 zxj 重启spad进程
static int restart_spad(ztn_srv_options_t * const opts);

//2023.7.19 zxj 将进程的PID写入PID文件 PID即进程ID
static int write_pid_file(ztn_srv_options_t *opts);

//2023.7.19 zxj 处理收到的信号
static int handle_signals(ztn_srv_options_t *opts);

//2023.7.19 zxj 设置PID相关的配置
static void setup_pid(ztn_srv_options_t *opts);

//2023.7.19 zxj 根据配置文件中的设置初始化重放缓存
static void init_digest_cache(ztn_srv_options_t *opts);

//2023.7.19 zxj 设置本地化语言环境
static void set_locale(ztn_srv_options_t *opts);
//2023.7.19 zxj 获取正在运行spa进程的PID
static pid_t get_running_pid(const ztn_srv_options_t *opts);
#if AFL_FUZZING
//2023.7.19 zxj 从文件中读取加密的SPA数据包，并对其进行解码和处理，AFL是一个知名的模糊测试工具，用于发现软件程序中的漏洞和错误
static void afl_enc_pkt_from_file(ztn_srv_options_t *opts);
//2023.7.19 zxj 从标准输入中读取SPA数据包，并进行解码和处理
static void afl_pkt_from_stdin(ztn_srv_options_t *opts);
#endif

#if HAVE_LIBFIU
//2023.7.19 zxj 启用故障注入
static void enable_fault_injections(ztn_srv_options_t * const opts);
#endif
...
main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        */
        ...
        /* 
         * 2023.7.19 zxj 终止当前正在运行的spad进程
        */
        if(opts.kill == 1)
            clean_exit(&opts, NO_FW_CLEANUP, stop_spad(&opts));

        /*2023.7.19 zxj 当前运行的spad进程的状态
        */
        if(opts.status == 1)
            clean_exit(&opts, NO_FW_CLEANUP, status_spad(&opts));

        /* 2023.7.19 zxj 重启进程
        */
        if(opts.restart == 1)
            clean_exit(&opts, NO_FW_CLEANUP, restart_spad(&opts));
        ...
        */
        if(!opts.afl_fuzzing
                && ! check_dir_path((const char *)opts.config[CONF_SPA_RUN_DIR], "Run", 0))
        ...
        */
        ...
        /* 根据 spad.conf 文件初始化防火墙规则处理程序，但对于 iptables 防火墙，
         * 暂时不刷新任何规则或创建任何链。这使得我们可以在 --fw-list 模式下通过 fw_rules_dump() 打印当前防火墙规则，
         * 而不会改变任何现有 spad 进程的规则。
        ...
        */
        ...
        /* 2023.7.19 zxj 准备防火墙——例如，清除旧规则并(对于iptables)创建spa链
        */
        ...
        */
        ...
        /* 如果设置了TCP服务器选项，在这里启动它。注意，在这种模式下，spad仍然通过libpcap获取SPA数据包。
         * 如果你想只使用UDP而不依赖libpcap，那么需要用——enable-udp-server来编译spa。
         * 请注意，即使在spad链接libpcap时，UDP服务器也可以运行，
    ...
    log_msg(LOG_INFO, "Shutting Down spad.");
...
//2023.7.19 zxj 用于设置 spad 服务器的区域设置（locale）。区域设置包括语言、日期时间格式、货币等本地化相关信息
static void set_locale(ztn_srv_options_t *opts)
{
    char               *locale;
...
static void afl_enc_pkt_from_file(ztn_srv_options_t *opts)
{
    ...
    if(fp != NULL)
    {
        ...
        fclose(fp);
		//ztn_new函数用于初始化spa上下文
        ztn_new(&decrypt_ctx);
		
		//ztn_afl_set_spa_data函数将读取到的数据包设置为加密数据
        res = ztn_afl_set_spa_data(decrypt_ctx, (const char *)enc_spa_pkt,
                enc_msg_len);

		//解码SPA数据，spatest是指定的解密密钥
        if(res == ZTN_SUCCESS)
            res = ztn_decrypt_spa_data(decrypt_ctx, "spatest",
                    strlen("spatest"));
        ...
        if(res == ZTN_SUCCESS)
            ...
            log_msg(LOG_ERR, "Error (%d): %s", res, ztn_errstr(res));

		//销毁解码上下文
        ztn_destroy(decrypt_ctx);

		//检查解码结果并打印成功和失败信息
        if(res == ZTN_SUCCESS)
        {
            log_msg(LOG_INFO, "SPA packet decode: %s", ztn_errstr(res));
            es = EXIT_SUCCESS;
        }
        else
        {
            log_msg(LOG_ERR, "Could not decode SPA packet: %s", ztn_errstr(res));
            es = EXIT_FAILURE;
        }
    }
    else
...
static void afl_pkt_from_stdin(ztn_srv_options_t *opts)
{
    ...
    if(fp != NULL)
    {
		...
		//设置spa的编码数据。参数分别是spa上下文、数据包字符串、数据包字符串的长度、起始索引(0)和SHA256哈希算法
        res = ztn_set_encoded_data(decode_ctx, (char *) spa_pkt,
                strlen((char *)spa_pkt), 0, ZTN_DIGEST_SHA256);

		//通过ztn_set_spa_data将SPA数据设置到上下文对象中，spa程序可以在后续的操作中使用这些数据，例如加密、解密、验证等
        if(res == ZTN_SUCCESS)
            res = ztn_set_spa_data(decode_ctx, (const char *) spa_pkt);
		//对数据进行解码
        if(res == ZTN_SUCCESS)
            res = ztn_decode_spa_data(decode_ctx);
		//将spa上下文的状态转储到dump_buf中
        if(res == ZTN_SUCCESS)
...
/* 2023.7.20 zxj
   这个函数用于初始化消息摘要缓存，以便 spa 服务器可以在接收到客户端消息摘要后进行验证，
...
static void setup_pid(ztn_srv_options_t *opts)
{
    ...
    */
    ...
    {
        /*它调用 get_running_pid 函数获取正在运行的 spad 进程的 PID。
        ...
        */
        ...
        else
        {
             ...
             *如果 PID 文件已经存在，并且成功读取到旧的 PID 值，说明已经有一个 spad 进程在运行，输出相应的错误信息并退出程序
        	*/
            old_pid = write_pid_file(opts);
            if(old_pid > 0)
            {
                fprintf(stderr,
                    "[*] An instance of spad is already running: (PID=%i).\n", old_pid
                );
...
static int restart_spad(ztn_srv_options_t * const opts)
{
    ...
    if(old_pid > 0)
    {
        ...
        if(res == 0)
        {
            fprintf(stdout, "Sent restart signal to spad (pid=%i)\n", old_pid);
            return EXIT_SUCCESS;
        }
        else
        {
        	//失败返回EXIT_FAILURE(-1)
            perror("Unable to send signal to spa: ");
            return EXIT_FAILURE;
        }
    }

    fprintf(stdout, "No running spad detected.\n");
    return EXIT_FAILURE;
}
...
static int status_spad(ztn_srv_options_t * const opts)
{
    ...
    //PID>0，检测到有spa的进程，返回EXIT_SUCCESS，打印进程信息
    if(old_pid > 0)
    {
        fprintf(stdout, "Detected spad is running (pid=%i).\n", old_pid);
        return EXIT_SUCCESS;
    }

    //PID=0，无spa进程，返回EXIT_FAILURE
    fprintf(stdout, "No running spad detected.\n");
    return EXIT_FAILURE;
}
...
static int handle_signals(ztn_srv_options_t *opts)
{
    ...
    if(got_signal) {
        ...
        if(got_sighup)
        {
			...
			//释放旧的配置并重新读取配置文件
            free_configs(opts);
			//并且如果TCP服务器正在运行，会向其发送SIGTERM信号终止进程。然后函数返回0，表示spad进程不会退出。
            if(opts->tcp_server_pid > 0)
                kill(opts->tcp_server_pid, SIGTERM);
            usleep(1000000);
            got_sighup = 0;
            rv = 0;  /* this means spad will not exit */
        }
		//SIGINT信号，表示需要退出进程，函数会输出相应的日志信息，然后返回1，表示spad进程需要退出
        else if(got_sigint)
        {
            log_msg(LOG_WARNING, "Got SIGINT. Exiting...");
            got_sigint = 0;
        }
		//SIGTERM信号，表示需要退出进程，函数会输出相应的日志信息，然后返回1，表示spad进程需要退出。
        else if(got_sigterm)
        {
...
static int stop_spad(ztn_srv_options_t * const opts)
{
    ...
    if(old_pid > 0)
    {
        ...
        if(res == 0 && is_err != 0)
        {
            fprintf(stdout, "Killed spad (pid=%i)\n", old_pid);
            return EXIT_SUCCESS;
        }
        else
        {
            ...
            */
            for (sleep_num = 0; sleep_num < 4; sleep_num++) {
                ...
                if(is_err != 0)
                {
                    fprintf(stdout, "Killed spad (pid=%i) via SIGTERM\n",
                            old_pid);
                    return EXIT_SUCCESS;
                }
                sleep(1);
    ...
    fprintf(stderr, "No running spad detected.\n");
    return EXIT_FAILURE;
}
...
*/
static void
daemonize_process(ztn_srv_options_t * const opts)
{
    pid_t pid, old_pid;
...
//将进程的PID写入PID文件
static int
write_pid_file(ztn_srv_options_t *opts)
{
    ...
    */
    ...
    op_fd = open(
        opts->config[CONF_SPA_PID_FILE], O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR
    );
    ...
    */
    ...
    log_msg(LOG_DEBUG, "[+] Writing my PID (%i) to the lock file: %s",
        my_pid, opts->config[CONF_SPA_PID_FILE]);
...
//获取正在运行spa进程的PID
static pid_t
get_running_pid(const ztn_srv_options_t *opts)
{
    ...
    op_fd = open(opts->config[CONF_SPA_PID_FILE], O_RDONLY);
    ...
    if(verify_file_perms_ownership(opts->config[CONF_SPA_PID_FILE], op_fd) != 1)
    {
...
//启用故障注入
static void
enable_fault_injections(ztn_srv_options_t * const opts)
{
