...
*/
static const char*
spad_errstr(const int err_code)
{
    switch (err_code)
    {
        case 0:
        ...
        case SPA_MSG_ZTN_CTX_ERROR:
            return("Error creating ZTN context for incoming data.");
    ...
    return("Undefined/unknown spad Error");
}

/* 尝试确定错误代码类型并发送相应的响应。基本上，如果它不是spad错误代码，就假定它是libztn的错误。
*/
const char*
get_errstr(const int err_code)
{
    if(! IS_SPAD_ERROR(err_code))
        return(ztn_errstr(err_code));

    return(spad_errstr(err_code));
}

/* 打印所有服务器错误（来自server/spad_errors.c）到标准输出
*/
void
