...
//检查防火墙链中是否存在与给定参数匹配的规则
static int
rule_exists_no_chk_support(const fko_srv_options_t * const opts,
        const struct fw_chain * const fwc,
        const unsigned int proto,//协议
        const char * const srcip,//源IP
        const char * const dstip,//目标IP
        const unsigned int port,//端口号
        const char * const natip,//NAT IP地址
        const unsigned int nat_port,//NAT端口号
        const unsigned int exp_ts) //过期时间戳
{
    int     rule_exists=0;
...
处理 SPA（Service Provider Authentication）请求
*/
int
        ...
        const acc_stanza_t * const acc, spa_data_t * const spadat)
{
    ...
    */
     ...
     * for NAT access requests.
     记住第一个proto/port组合，以防NAT访问请求需要它们
    */
    fst_proto = ple->proto;
    fst_port  = ple->port;
    ...
    */
      ...
      || acc->force_nat)
    {
        ...
        {
            ndx = strchr(spadat->nat_access, ','); //返回的是指针
            str_len = strcspn(spadat->nat_access, ",");//返回的是整型，这两个功能类似
            if((ndx != NULL) && (str_len <= MAX_HOSTNAME_LEN))//70
            {
                strlcpy(nat_dst, spadat->nat_access, str_len+1);//加1是因为最后要以空字符'\0'结尾
                if(! is_valid_ipv4_addr(nat_dst, str_len))
                {
                    if(strncasecmp(opts->config[CONF_ENABLE_NAT_DNS], "Y", 1) == 0)
                    {
                        if (!is_valid_hostname(nat_dst, str_len))//验证主机名
                        {
                ...
                {
                    strlcpy(nat_ip, nat_dst, MAX_IPV4_STR_LEN);//16
                }
				//端口号
